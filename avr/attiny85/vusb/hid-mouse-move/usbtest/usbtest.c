...
int main(int argc, char **argv)
{
   ...
   if(argc < 2) {
        printf("Usage:\n");
        printf("usbtext.exe left\n");
        printf("usbtext.exe right\n");
        printf("usbtext.exe up\n");
        printf("usbtext.exe down\n");
        exit(1);
   }
   ...
   if(strcmp(argv[1], "left") == 0)
     {
        nBytes = usb_control_msg(handle,
                                 ...
                                 USB_LEFT_MOVE, 0, 0, (char *)buffer, sizeof(buffer), 5000);
     }
   else if(strcmp(argv[1], "right") == 0)
     {
        nBytes = usb_control_msg(handle,
                                 USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN,
                                 USB_RIGHT_MOVE, 0, 0, (char *)buffer, sizeof(buffer), 5000);
     }
   else if(strcmp(argv[1], "up") == 0)
     {
        nBytes = usb_control_msg(handle,
                                 USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN,
                                 USB_UP_MOVE, 0, 0, (char *)buffer, sizeof(buffer), 5000);
     }
   else if(strcmp(argv[1], "down") == 0)
     {
        nBytes = usb_control_msg(handle,
                                 USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN,
                                 USB_DOWN_MOVE, 0, 0, (char *)buffer, sizeof(buffer), 5000);
     }
