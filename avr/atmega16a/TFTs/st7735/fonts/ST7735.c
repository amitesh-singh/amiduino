...
unsigned char
DrawChar(unsigned char charToWrite, unsigned char poX,
         unsigned char poY, unsigned char size, unsigned int fgcolor)
{
   int i;
   charToWrite -= 32;
             ...
             if((temp>>f) & 0x01)
               {
                  if(size==1)
                  {
                   ...
                   {
                      FilledRec(poX+i*size, poY+f*size, size, size, fgcolor);
                   }
               }
          }
   }
...
int main()
{
   ...
   DrawString("Humidity: 16 %", 20, 60, 1, BLUE);
   DrawString("Amitesh Singh", 5, 100, 1, GOLD);
   DrawString("Samsung R&D", 10, 110, 2, SILVER);
