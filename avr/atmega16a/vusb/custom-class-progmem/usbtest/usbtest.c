...
int main(int argc, char **argv) {
     ...
     if(argc < 2) {
          printf("Usage:\n");
          printf("usbtext.exe drivername\n");
          printf("usbtext.exe driverver\n");
          printf("usbtext.exe driverauth\n");
          exit(1);
     }
     ...
     if(strcmp(argv[1], "drivername") == 0)
       {
          nBytes = usb_control_msg(handle, 
                                   USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN, 
                                   USB_DRIVER_NAME, 0, 0, (char *)buffer, sizeof(buffer), 5000);
          printf("Got %d bytes: Driver name %s\n", nBytes, buffer);
       }
     else if(strcmp(argv[1], "driverver") == 0)
       {
          nBytes = usb_control_msg(handle, 
                                   USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN, 
                                   USB_DRIVER_VER, 0, 0, (char *)buffer, sizeof(buffer), 5000);
          printf("Got %d bytes: Driver version: %s\n", nBytes, buffer);
       }
     else if(strcmp(argv[1], "driverauth") == 0)
       {
          nBytes = usb_control_msg(handle,
                                   USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN, 
                                   USB_DRIVER_AUTH, 0, 0, (char *)buffer, sizeof(buffer), 5000);
          printf("Got %d bytes: Author: %s\n", nBytes, buffer);
       }
